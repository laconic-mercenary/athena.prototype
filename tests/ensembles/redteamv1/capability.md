...
- **Flow:** OSINT first — an OSINT specialist reads the company's public site to find its tech
  stack and a public code repository; a GitHub specialist inspects that repo's commit history
  and recovers a host/IP leaked in an old commit. Recon then scans and enumerates that
  discovered target.
...
- **Adequate when:** a target was discovered from OSINT and confirmed reachable with at least
  one open port and either CVE candidates or a surface assessment.
- **Retry if:** OSINT yields no repo/host (ask_operator) or nmap returns no open ports on the
  discovered target (retry with a slower/discovery scan).
...
- **Mode:** Compare — three `exploit_planner` specialists run in parallel (two Claude Haiku
  instances at different temperatures; one Foundation-Sec instance). Leader selects the most
  technically credible plan via `select_result`.
...
- **Elements:** `flask_exploiter` (non-interactive RCE via http_post; also holds get_shell as a
  fallback), `redis_operator` (activated once a datastore is discovered), and `shell_operator`
  (interactive-shell fallback). The leader matches specialists to the technology it encounters
  and prefers non-interactive RCE.
...
## Ask the operator if
- OSINT finds no public repository, or no host/IP is recoverable from it (do not invent an IP).
...
- `httpx` — HTTP client used by http_get, http_post, github_commits, get_shell
- `pwntools` — for the get_shell interactive-shell fallback only
- Foundation-Sec — configured via `provider: ollama` in the manifest (planning committee only)
- Outbound internet from the harness — to reach the OSINT domain and the GitHub API
